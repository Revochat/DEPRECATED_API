...
export const inviteCreate = async (req: express.Request, res: express.Response) => { // Create an invite for a server
    try {
        ...
        if (!server_id || !token || !uses || !expires_at || 
            ...
            token.length < UTILS.CONSTANTS.USER.TOKEN.MIN_LENGTH || token.length > UTILS.CONSTANTS.USER.TOKEN.MAX_LENGTH || isNaN(parseInt(server_id)) || isNaN(Date.parse(expires_at)) || isNaN(parseInt(uses))
