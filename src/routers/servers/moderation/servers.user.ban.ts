...
export const userBan = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        if (!UTILS.FUNCTIONS.CHECK.SERVER.PERMISSIONS(User, Server, UTILS.CONSTANTS.SERVER.PERMISSIONS.ADMIN)) throw "You do not have permission to ban users"
        ...
        if (!Server.banned_users) Server.banned_users = [] // if banned_users is undefined, set it to an empty array
        Server.banned_users.push(parseInt(user_id))

        Emitter.emit("updateBan", Server)
